...
const { recommendInternships } = require('./controllers/recommendationController');
...
app.use('/pdf.worker.min.js', express.static(path.join(__dirname, 'node_modules/pdfjs-dist/build/pdf.worker.min.js')));
...
const fetchAndUpdateApplications = async (req, res) => {
    ...
    try {
        const { platforms = ['Indeed', 'JSearch', 'Remotive', 'Internshala'], skills = 'Software Development', field = 'IT', minStipend = '10000', maxStipend = '100000' } = req.body;
        
        const response = await fetch('http://localhost:5000/api/jobs', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ platforms, skills, field, minStipend, maxStipend })
        });
...
app.post('/api/generate-summary', async (req, res) => {
    ...
    if (!resumeContent || !jobRequirements) {
        return res.status(400).json({ error: 'Resume content and job requirements are required' });
    }
...
app.post('/api/generate-improved-resume', (req, res) => {
    ...
    try {
        const doc = new PDFDocument({ size: 'A4', margins: { top: 30, bottom: 30, left: 50, right: 50 } });
        ...
        const sections = {
            header: text.match(/^[\s\S]*?(?=Professional Summary)/i) || ['Your Name\nYour Email\nYour Phone Number'],
            summary: text.match(/(Professional Summary)[\s\S]*?(?=\n[A-Z]|\n\n|$)/i) || ['Professional Summary\nNo summary provided.'],
            experience: text.match(/(Experience|Work Experience|Professional Experience)[\s\S]*?(?=\n[A-Z]|\n\n|$)/i) || ['Experience\nNo experience provided.'],
            skills: text.match(/(Skills|Technical Skills|Key Skills|Proficiencies|Languages|Technologies & Tools)[\s\S]*?(?=\n[A-Z]|\n\n|$)/i) || ['Skills\nNo skills provided.'],
            education: text.match(/(Education)[\s\S]*?(?=\n[A-Z]|\n\n|$)/i) || ['Education\nNo education provided.'],
            projects: text.match(/(Projects|Project Work)[\s\S]*?(?=\n[A-Z]|\n\n|$)/i) || ['Projects\nNo projects provided.'],
            certifications: text.match(/(Certifications\/Achievements)[\s\S]*?(?=\n[A-Z]|\n\n|$)/i) || ['Certifications/Achievements\nNo certifications provided.']
        };

        if (sections.header) {
            const headerLines = sections.header[0].split('\n').filter(line => line.trim());
            doc.fontSize(16).font('Helvetica-Bold').text(headerLines[0] || 'Your Name', { align: 'center' });
            doc.moveDown(0.2);
        ...
        if (sections.summary) {
            let summaryText = sections.summary[0].replace(/(Professional Summary)[\s:]+/i, '').trim();
            if (!summaryText || summaryText === 'No summary provided.') {
                summaryText = 'A dedicated professional with a strong foundation in relevant skills, eager to contribute to innovative projects.';
            }
            addSection('Professional Summary', summaryText, false, 9);
        }

        if (sections.experience) {
            const experienceText = sections.experience[0].replace(/(Experience|Work Experience|Professional Experience)[\s:]+/i, '').trim();
            const experienceList = experienceText.split('\n').filter(line => line.trim());
            addSection('Experience', experienceList, true, 9);
        }

        if (sections.skills) {
            let skillsText = sections.skills[0].replace(/(Skills|Technical Skills|Key Skills|Proficiencies|Languages|Technologies & Tools)[\s:]+/i, '').trim();
            let existingSkills = skillsText.split(/,\s*|\n/).map(skill => skill.trim()).filter(Boolean);
            ...
            existingSkills = existingSkills.concat(newSkills.map(skill => `${skill} (In Progress)`));
            addSection('Skills', existingSkills, true, 9);
        }

        if (sections.education) {
            const educationText = sections.education[0].replace(/(Education)[\s:]+/i, '').trim();
            const educationList = educationText.split('\n').filter(line => line.trim());
            addSection('Education', educationList, true, 8);
        }

        if (sections.projects) {
            const projectsText = sections.projects[0].replace(/(Projects|Project Work)[\s:]+/i, '').trim();
            const projectList = projectsText.split('\n').filter(line => line.trim());
            addSection('Projects', projectList, true, 8);
        }

        if (sections.certifications) {
            const certText = sections.certifications[0].replace(/(Certifications\/Achievements)[\s:]+/i, '').trim();
            const certList = certText.split('\n').map(cert => cert.trim()).filter(Boolean);
