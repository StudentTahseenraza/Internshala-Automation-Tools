...
function ApplicationTracker() {
  ...
  const getSummary = () => {
    const summary = {
      ...
      notApplicable: applications.filter(
        (app) => app.status === "not applicable"
      ).length,
    };
    return summary;
  };
  ...
  const fetchApplications = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE}/api/fetch-applications`,
        {
          timeout: 5000,
        }
      );
      ...
      setError(
        "Failed to load application tracking list. Please check your network connection and try again later."
      );
    } finally {
      setLoading(false);
    }
            ...
            <tbody>
              {filteredApplications.map((app) => (
                <tr key={app.id}>
                  ...
                  <td>
                    <span
                      className={`status-badge status-${app.status.replace(
                        " ",
                        "-"
                      )}`}
                    >
