...
const InternshipRecommendation = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const searchQuery = formData.skills
      .split(",")[0]
      .trim()
      .replace(/\s+/g, "-");
    window.open(
      `https://internshala.com/internships/${searchQuery}-internship`,
      "_blank"
    );
    ...
    try {
      const response = await axios.post(
        `${API_BASE}/api/recommend-internships`,
        data,
        {
  ...
  const handleApply = async (link) => {
    setLoading(true);
    try {
      const response = await axios.post(`${API_BASE}/api/auto-apply`, { link });
      alert("Successfully applied!");
      console.log("Apply response:", response.data);
    } catch (err) {
      setError(
        "Could not apply automatically. " + (err.response?.data?.error || "")
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="internship-container">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit" disabled={loading}>
          {loading ? "Fetching Internships..." : "Get Recommendations"}
        </button>
      </form>
        ...
        <div className="result">
          ...
          <ul>
            {recommendations.map((rec, i) => (
              <li key={i}>
                <strong>{rec.title}</strong> at {rec.company}
                <br />
                Stipend: {rec.stipend}
                <br />
                Score: {rec.score}%<br />
